    ...
    public function inventoryStock()
    {
        ...
        $i= 0;
            foreach ($stocks as $stock)
            {
                ...
                $data[$i+1]['total_value'] = $stock->purchase_cost*$stock->stock_quantity;
                $data[$i+1]['selling_cost'] = $stock->selling_cost;
                $data[$i+1]['supplier_name'] = $stock->supplier_name;
                $i += 50000;
                $grandTotal = $grandTotal + ($stock->purchase_cost*$stock->stock_quantity);
            }
        $data[$i+1]['key'] = '';
        $data[$i+1]['stock_name'] = '';
        $data[$i+1]['stock_quantity'] = '';
        $data[$i+1]['purchase_cost'] = 'Total';
        $data[$i+1]['total_value'] = $grandTotal;
        $data[$i+1]['selling_cost'] = '';
        $data[$i+1]['supplier_name'] = '';
...
// dd($request->all());
        ...
        // dd($date_from, 'hi');
        // $testDate = date('Y-m-d', 1603648800);
        // dd($testDate);
        $date_to = strtotime($to); // Convert date to a UNIX timestamp
        $total_amount = 0;
        if($request->debit_for == 'all')
        {
            ...
            if($request->debit_for == 'rent')
            {
                
                $transactions = Transaction::where('type', $request->type)->where('payment_for', $request->debit_for)->whereBetween(DB::raw('DATE(transaction_date)'), array($from, $to))->get();
                // dd($transactions);
            }
            else
            {
        ...
        // dd($transactions[0]->transaction_date);
        // dd($transactionsOnDays);
        for ($i=$date_from; $i<=$date_to; $i+=86400) {
            $date_this = date("Y-m-d", $i);
            $data[$i]['date'] = $date_this;
...
// dd($date_this);
            if(array_key_exists($date_this, $transactionsOnDays->toArray()))
            {
                foreach ($transactionsOnDays[$date_this] as $transactionOnDay)
                {
                    ...
                    if($request->debit_for == 'rent')
                    {
                        // dd($transactionOnDay->rent->vehicle->registration_number);
                        $data[$i]['notes'] = $transactionOnDay->rent->vehicle->registration_number;
                        $data[$i]['driver'] = $transactionOnDay->rent->driver->user->name;
                    }
                    else{
