...
class VehicleAllExport implements FromArray,  withHeadings, WithColumnFormatting, ShouldAutoSize
{
    private $vehicles;
    ...
    */
    ...
    public function array(): array
    {
        ...
        foreach ($this->vehicles as $key => $vehicle){
            ...
            $data[$key]['profit'] = number_format(($vehicle->rents->sum('collection')-($vehicle->rents->sum('due')+$vehicle->cases->sum('penalty')+$vehicle->sales->sum('sales_total')+$vehicle->damages->sum('amount'))), 2);
        }
        return $data;
    }
}
