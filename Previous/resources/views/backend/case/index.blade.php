...
{{--                    {{dd($rents[0]->vehicle)}}--}}
                    ...
                    <tr>
                        ...
                        <td class="desc">{{$case->case_id}}</td>
                        <td>
                            <span class="block-email">{{$case->rent->vehicle->registration_number}}</span>
                        </td>
                        <td>{{$case->rent->driver->user->name }} </td>

                        <td>{{$case->penalty}}</td>
                        @php
                            $caseDate = now()->diffInDays(Carbon\Carbon::parse($case->last_date), false);
                        @endphp
                        <td >
                            @if($caseDate < 18 && $caseDate >= 7)
                                <span class="shadow-none badge badge-warning">{{date('d-m-Y', strtotime($case->last_date))}}</span>
                            @elseif($caseDate < 7)
                                <span class="shadow-none badge badge-danger">{{date('d-m-Y', strtotime($case->last_date))}}</span>
                            @else
                                <span class="shadow-none badge badge-success">{{date('d-m-Y', strtotime($case->last_date))}}</span>
                            @endif
                        </td>
                        <td>
                            @if($case->doc_status == 0)
                                <span class="shadow-none badge badge-danger">{{$case->doc_type}}</span>
                            @else
                                <span class="shadow-none badge badge-success">{{$case->doc_type}}</span>
                            @endif
                        ...
                        <td>
                            <div class="btn-group">
                                @if($case->payment_status == 1)
                                    @if($case->doc_status == 0)
                                    <a type="button" href="{{route('case.update', $case->id)}}" onclick="return confirm('Are you sure?')" class="btn btn-dark btn-sm">Doc Receive</a>
                                    @else
                                        {{'Doc Received'}}
                                    @endif
                                @else
                                    <a type="button" class="btn btn-dark btn-sm" href="#"  onClick="alert('Payment Not Issued')">Doc Receive</a>
                                @endif
